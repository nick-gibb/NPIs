...
df_c["region"].fillna('All', inplace=True) # If the region field is blank, it applies to all Canada
...
for index, row in df_can.iterrows():
    ...
    stringency_score = row[string_cols].sum() # Get the sum score for that intervention 
...
for index, row in df_can.iterrows():
    ...
    r = requests.post('https://api.deepl.com/v2/translate', data={'auth_key':DEEPL_API_KEY, 'target_lang':'FR', 'text': source_title})
